...
def get_obs_list_from_folder(
    ...
    """get a list of gmw observations within a folder.
    ...
    """
    ns = {"brocommon": "http://www.broservices.nl/xsd/brocommon/3.0",
...
def get_metadata_from_gmw(path_xml, tube_nr, full_meta=False):
    ...
    if full_meta:
        if (tree.find('brocommon:deliveryAccountableParty', ns) is not None):
            ...
            meta["deliveryAccountableParty"] = tree.find(
                'brocommon:deliveryAccountableParty', ns).text
    ...
    if full_meta:
        if (GMW_c.find('isgmw:objectIdAccountableParty', ns) is not None):
            meta["objectIdAccountableParty"] = GMW_c.find(
                'isgmw:objectIdAccountableParty', ns).text
        ...
        if (GMW_c.find('isgmw:constructionStandard', ns) is not None):
            meta["constructionStandard"] = GMW_c.find('isgmw:construction'
                                                      'Standard', ns).text
    ...
    if full_meta:
        if (GMW_c.find('isgmw:deliveredLocation//'
                       'gmwcommon:horizontalPositioningMethod', ns) is not None):
            meta["horizontalPositioningMethod"] = GMW_c.find(
                'isgmw:deliveredLocation//'
                'gmwcommon:horizontalPositioningMethod', ns).text
        if (GMW_c.find('isgmw:deliveredVerticalPosition//'
                       'gmwcommon:localVerticalReferencePoint', ns) is not None):
            meta["localVerticalReferencePoint"] = GMW_c.find(
                'isgmw:deliveredVerticalPosition//'
                'gmwcommon:localVerticalReferencePoint', ns).text
        if (GMW_c.find('isgmw:deliveredVerticalPosition//'
                       'gmwcommon:offset', ns) is not None):
            meta["deliveredVerticalPosition_offset"] = float(GMW_c.find(
                'isgmw:deliveredVerticalPosition//'
                'gmwcommon:offset', ns).text)
        if (GMW_c.find('isgmw:deliveredVerticalPosition//'
                       'gmwcommon:groundLevelPositioningMethod', ns) is not None):
            meta["groundLevelPositioningMethod"] = GMW_c.find(
                'isgmw:deliveredVerticalPosition//'
                'gmwcommon:groundLevelPositioningMethod', ns).text

    # ground_level
    glp_xml = GMW_c.find('isgmw:deliveredVerticalPosition//'
                         'gmwcommon:groundLevelPosition', ns)
    vert_datum = GMW_c.find('isgmw:deliveredVerticalPosition//'
                            'gmwcommon:verticalDatum', ns).text
    meta["unit"] = glp_xml.attrib["uom"] + " " + vert_datum
    if glp_xml.attrib["uom"].lower() != 'm':
        logger.info(f'groundlevel unit is unexpected {glp_xml.attrib["uom"]}, '
                    'm is expected')
    ...
    if full_meta:
        ...
        if tube.find('isgmw:artesianWellCapPresent', ns) is not None:
            meta["artesianWellCapPresent"] = tube.find(
                'isgmw:artesianWellCapPresent', ns).text
        if tube.find('isgmw:sedimentSumpPresent', ns) is not None:
            meta["sedimentSumpPresent"] = tube.find(
                'isgmw:sedimentSumpPresent', ns).text
        if tube.find('isgmw:numberOfGeoOhmCables', ns) is not None:
            meta["numberOfGeoOhmCables"] = int(tube.find(
                'isgmw:numberOfGeoOhmCables', ns).text)
        if tube.find('isgmw:tubeTopDiameter', ns) is not None:
            ...
            meta["tubeTopDiameter_unit"] = tube.find(
                'isgmw:tubeTopDiameter', ns).attrib["uom"]
        ...
        if tube.find('isgmw:tubeTopPositioningMethod', ns) is not None:
            meta["tubeTopPositioningMethod"] = tube.find(
                'isgmw:tubeTopPositioningMethod', ns).text
        if tube.find('isgmw:materialUsed//'
                     'gmwcommon:tubePackingMaterial', ns) is not None:
            meta["tubePackingMaterial"] = tube.find(
                'isgmw:materialUsed//gmwcommon:tubePackingMaterial', ns).text
        if tube.find('isgmw:materialUsed//gmwcommon:tubeMaterial', ns) is not None:
            meta["tubeMaterial"] = tube.find(
                'isgmw:materialUsed//gmwcommon:tubeMaterial', ns).text
        ...
        if tube.find('isgmw:screen//isgmw:sockMaterial', ns) is not None:
            meta["sockMaterial"] = tube.find(
                'isgmw:screen//isgmw:sockMaterial', ns).text
    ...
    if screenLength_unit != "m":
        logger.info(f'screenLength unit is unexpected {screenLength.attrib["uom"]},'
                    'm expected')
    ...
    if plainTubePartLength_unit != "m":
        logger.info('plainTubePartLength unit is unexpected'
                    f'{plainTubePartLength_unit}, m expected')

    if tube.find("isgmw:sedimentSumpPresent", ns).text.lower() in ['ja', 'yes']:
        sedimentSumpLength_xml = tube.find('isgmw:sedimentSump//'
                                           'gmwcommon:sedimentSumpLength', ns)
        ...
        if sedimentSumpLength_unit != "m":
            logger.info('sedimentSumpLength unit is unexpected'
                        f'{sedimentSumpLength_unit}, m expected')

        if full_meta:
            meta["sedimentSumpLength"] = sedimentSumpLength

    if ('tube_top' in meta) and ('plainTubePartLength' in locals()) and \
            (tube_top_unit == plainTubePartLength_unit):
